"""
Double DQN training script for LunarLander-v3 using Stable-Baselines3 (SB3)
...
Note: SB3's default DQN implementation uses Double DQN principles internally.
"""
...
# ---------- Parse CLI Arguments ----------
parser = argparse.ArgumentParser(description="Train Double DQN on LunarLander-v3 with configurable params.")
...
# ---------- Paths ----------
MODEL_DIR = "models_ddqn" # Updated directory name
MODEL_PATH = os.path.join(MODEL_DIR, "ddqn_lunarlander_v3.zip") # Updated model name
TB_LOG = "./tb_ddqn_lunarlander_v3" # Updated TensorBoard log name
...
def train_dqn(total_steps, stage_size, lr):
    ...
    if os.path.exists(MODEL_PATH):
        ...
        model = DQN(
            ...
            device=DEVICE,
            # SB3 implements Double DQN logic by default, no flag needed for modern versions.
        )
    ...
    plt.title("📈 Double DQN Training Progress on LunarLander-v3")
    plt.xlabel("Training Stage")
    plt.ylabel("Mean Reward")
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(MODEL_DIR, "training_reward_plot_ddqn.png"))
    print(f"📊 Saved training progress plot to {MODEL_DIR}/training_reward_plot_ddqn.png")
...
def evaluate_and_report(model, n_eval_episodes=20, render=False):
    ...
    plt.title("🚀 Evaluation Results (Episode Outcomes) DDQN")
    plt.ylabel("Count")
    plt.xticks(rotation=30)
    plt.tight_layout()
    plt.savefig(os.path.join(MODEL_DIR, "evaluation_summary_ddqn.png"))
    print(f"📊 Saved evaluation summary plot to {MODEL_DIR}/evaluation_summary_ddqn.png")
