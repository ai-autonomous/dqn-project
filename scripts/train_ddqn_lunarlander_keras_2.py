...
# ----------------- Agent -----------------
class DDQNAgent:
    def __init__(self, state_dim: int, action_dim: int, lr: float = LR, buffer_size: int = BUFFER_SIZE):
...
# ----------------- Main Training -----------------
def train(episodes: int = MAX_EPISODES) -> Tuple["DDQNAgent", list, list]:
    set_seed(SEED)
    ...
    while episode < episodes and frame < MAX_FRAMES:
        reset_ret = env.reset()
...
if __name__ == '__main__':
    print(f"Training on {ENV_NAME} | LR={LR} | Device={DEVICE}")
    agent, rewards, losses = train(episodes=MAX_EPISODES)
